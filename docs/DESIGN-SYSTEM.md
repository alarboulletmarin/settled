...
**Et la grille ne porte que le résumé.** L'écran du mois en comptait douze — une tuile par métrique, quatre soldes qui se ressemblent, deux répartitions, un capital restant dû, une liste d'échéances — et tout y avait le même poids sur deux écrans de défilement. Une grille bento répond à des questions de **montant**, d'un coup d'œil : ce à quoi on répond autrement n'y a pas sa place. Ce qui se **dérive** de ces chiffres descend en rangées (§6, `RowGroup`), ce qui appelle **un autre écran** s'y rend en une ligne, et ce qui est une **liste** devient une section en flux — une liste dont la hauteur vient d'un format se serre à un pixel par ligne pour y tenir.

Il en reste quatre, et leur ordre pave la grille sans un trou sur six colonnes : solde `2x2` (c1-2, deux rangées), revenus `2x1` (c3-4), charges `2x1` (c5-6), où part l'argent `4x2` sous les deux flux (c3-6). Sur deux colonnes les quatre s'empilent dans le même ordre, et les deux flux restent voisins — c'est la même phrase, on ne lit pas ce qui rentre sans ce qui sort. Le `4x2` de l'anneau ne coûte rien aux deux autres paliers, où il fait la même boîte qu'un `2x2` : c'est la correspondance format → colonnes qui change, pas la liste des formats.
...
C'est aussi la réponse à « **deux ou trois lectures voisines** », et l'écran du mois en fait deux usages. Prévisionnel, reste à vivre et capacité d'épargne se dérivent des mêmes deux flux à trois horizons différents : ils se lisent ensemble, sous une étiquette qui les nomme. Répartition et Crédits, eux, ne sont pas des lectures du mois mais deux écrans, résumés d'une ligne chacun. Dans les deux cas la rangée gagne quelque chose que la tuile ne pouvait pas donner : la `description` **passe à la ligne**, donc elle s'affiche à toutes les largeurs, là où une lecture secondaire de tuile plate disparaît sous son seuil (§5). Une microcopy qui n'existe qu'au-delà de 366px de fenêtre n'existe pas.
