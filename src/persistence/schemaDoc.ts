...
const RULES = [
  ...
  '**`shared` est une exception, jamais une copie de la règle.** Par défaut, est commune une sortie de nature `charge` ou `debt` que personne ne s’est attribuée. Ne pose `shared` que là où tu veux dire le contraire — typiquement `true` sur une charge qu’une personne règle mais que le foyer partage.',
  '**Une ligne est à quelqu’un, ou à tout le monde.** Une sortie sans `memberId` et hors partage sort du compte du foyer sans apparaître dans le mois de personne. Concrètement : toute entrée d’argent, tout versement d’épargne et toute charge non partagée veulent un `memberId`.',
  '**Le revenu d’une personne ne se déclare nulle part** : il se lit sur ses récurrences de nature `resource`. C’est ce qui répartit les charges communes au prorata — un foyer à deux revenus veut donc une récurrence de salaire *par personne*, avec son `memberId`.',
